        ...
        const content = () => {
            ...
            return <>
                <div className={`column ai-end ${classes.thisSession}`}>
                    <span className={`text-orange mb-05 font-size-sm-plus width-100 text-center py-05`} style={{backgroundColor: 'var(--tableHeader)'}}>{t("ActiveSessions.thisSession")}</span>
                    ...
                    <div className={`row jc-between width-100 py-05 px-1`}>
                        <div className={`col-40 column jc-center ai-center`}>
                            ...
                            {sessions.length > 0 ? <span className={`cursor-pointer text-red font-size-sm`} onClick={clickHandler}>{t("ActiveSessions.closeAllInusableSessions")}</span> : ""}
                        </div>
