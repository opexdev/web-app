...
const Overview = (props) => {
    ...
    const getOverviewData = (activePair)=>{
        ...
        axiosInstance.get('/api/v3/ticker/24hr',{
            params:{
    ...
    let lastDayBody = <div className={classes.content}>
        {overviewData.openTime > 0
            ?
            <Fragment>
                <p className="flex">{t('overview.change')}: <span className={`${overviewData.priceChangePercent > 0 ? "text-green" : "text-red"}`} style={{direction:"ltr"}}>{parseFloat(overviewData.priceChangePercent)} % </span> </p>
                <p>{t('min')}: <span className="text-red">{overviewData.lowPrice}</span> {t(`currency.${props.activePair.quote}`)}</p>
                <p>{t('max')}: <span className="text-green">{overviewData.highPrice}</span> {t(`currency.${props.activePair.quote}`)}</p>
                <p>{t('overview.volume')}: <span>{overviewData.volume} </span>{ t(`currency.${props.activePair.quote}`)}</p>
            </Fragment>
            :
            <div className="container flex ai-center jc-center flashit" style={{height:"100%"}}>در حال دریافت اطلاعات...</div>
        }
    </div>
