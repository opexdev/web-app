...
const Deposit = (props) => {
    ...
    const data = [
        {id: 1, title: t('Deposit.deposit'), body: t('Deposit.deposit') },
        {id: 2, title: t('Deposit.withdrawal'), body: t('Deposit.withdrawal') },
        {id: 3, title: t('Deposit.transfer'), body: t('Deposit.transfer') },

    ]

    return (
        <div className={`container card-background card-border column ${classes.container}`}>
            <AccordionBox title={t('Deposit.title')} content={data} safari={classes.safariFlexSize}/>
        </div>
    );
};
